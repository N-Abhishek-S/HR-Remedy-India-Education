...
function SheetOverlay({
  ...
  return (
    <DialogPrimitive.Overlay
      className={cn("fixed inset-0 z-50 bg-brand-navyDark/70 backdrop-blur-sm", className)}
      {...props}
    />
  );
}
...
const sheetVariants = cva(
  "fixed z-50 gap-4 bg-card p-6 shadow-soft transition ease-in-out data-[state=closed]:duration-300 data-[state=open]:duration-500",
  {
    variants: {
      side: {
